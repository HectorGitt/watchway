	...
	const handleLocation = async (loc: { lat: number; lng: number }) => {
		...
		try {
			...
			if (data.display_name) {
				...
				const simpleAddr = data.display_name
					.split(",")
					.slice(0, 3)
					.join(",");
		...
		<div className="min-h-screen bg-background text-foreground flex flex-col">
			...
			<div className="flex-1 container mx-auto px-4 py-8 max-w-lg">
				...
				<div className="flex gap-2 mb-8">
					<div
						className={`h-1 flex-1 rounded-full ${step !== "location" ? "bg-primary" : "bg-primary/50"}`}
					></div>
					<div
						className={`h-1 flex-1 rounded-full ${["photo", "details", "success"].includes(step) ? "bg-primary" : "bg-surface-highlight"}`}
					></div>
					<div
						className={`h-1 flex-1 rounded-full ${["details", "success"].includes(step) ? "bg-primary" : "bg-surface-highlight"}`}
					></div>
				</div>
				...
				{step === "photo" && (
					<FadeIn>
						...
						<p className="text-gray-400 mb-6 text-sm">
							Take a live photo of the hazard. Gallery uploads are
							disabled to ensure trust.
						</p>
						...
						<form onSubmit={handleSubmit} className="space-y-6">
							...
							<div>
								...
								<input
									type="text"
									required
									placeholder="e.g., In front of the Zenith Bank, near the bus stop..."
								...
								<p className="text-xs text-gray-500 mt-2">
									Help responders find it faster by describing
									nearby landmarks.
								</p>
							</div>
							...
							<div>
								...
								>
									...
									<option value="Deep Pothole">
										Deep Pothole
									</option>
									<option value="Road Washout / Erosion">
										Road Washout / Erosion
									</option>
									<option value="Bridge Damage">
										Bridge Damage / Expansion Joint
									</option>
									<option value="Missing Manhole Cover">
										Missing Manhole Cover
									</option>
									<option value="Blocked Drainage / Flooding">
										Blocked Drainage / Flooding
									</option>
									<option value="Collapsed Pole / Street Light">
										Collapsed Pole / Street Light
									</option>
				...
				{step === "success" && (
					<div className="text-center py-12 animate-fade-in">
						...
						<p className="text-gray-400 mb-8 max-w-xs mx-auto">
							Your report has been marked as{" "}
							<span className="text-yellow-500 font-bold">
								[Unverified]
							</span>{" "}
							until a nearby user confirms it.
						</p>
							...
							<div className="flex items-center gap-4 text-left">
								...
								<div className="flex-1">
									<p className="text-sm text-gray-300">
										Wait for the "Crowdsourced Validation"
										or share coordinate link.
									</p>
								</div>
							</div>
						</div>
