...
export default function HazardsPage() {
	...
	return (
		<div className="min-h-screen bg-background text-foreground pb-20 pt-20">
			{/* Header */}
			<div className="sticky top-20 z-40 bg-background/90 backdrop-blur border-b border-white/5 p-4">
									...
									<div className="group bg-surface border border-white/5 hover:border-primary/30 rounded-xl overflow-hidden transition-all duration-300 hover:shadow-2xl hover:shadow-primary/5 cursor-pointer h-full flex flex-col">
										<div className="relative h-40 bg-black/50 overflow-hidden">
											<div className="absolute top-2 right-2 z-10">
												<span
													className={`px-2 py-1 rounded text-[10px] font-bold uppercase tracking-wide backdrop-blur-md border ${
														report.status ===
														"verified"
											...
											<div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/80 to-transparent p-3 pt-8">
												<div className="flex items-center gap-1.5 text-xs text-white/90 font-medium">
													<span
														className={`w-2 h-2 rounded-full ${report.jurisdiction === "FEDERAL" || report.jurisdiction?.toUpperCase() === "LAGOS" ? "bg-red-500" : "bg-orange-500"}`}
													/>
													{report.jurisdiction ===
													"FEDERAL"
														? "LAGOS"
