...
export const api = {
	...
	getReports: async (filters: {
		...
		if (filters.hazard_type && filters.hazard_type !== "all")
			params.append("hazard_type", filters.hazard_type);
	...
	getProfile: async () => {
		...
		console.log(
			"API: Fetching profile with token",
	...
	getUsers: async (
		token: string,
		filters?: { role?: string; sort_by?: string },
	) => {
		const params = new URLSearchParams();
	...
	updateUserRole: async (token: string, userId: string, role: string) => {
		const res = await fetch(
			`${API_URL}/users/${userId}/role?role=${role}`,
			{
				method: "PUT",
				headers: { Authorization: `Bearer ${token}` },
			},
	...
	reviewApplication: async (
		...
		const res = await fetch(
			`${API_URL}/users/${userId}/review-application?status=${status}`,
			{
				method: "POST",
				headers: { Authorization: `Bearer ${token}` },
			},
